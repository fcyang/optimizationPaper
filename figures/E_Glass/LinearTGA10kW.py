...
fileName = 'Linear/E_GlassOldLinearNominal10KW_0_cone_Mass.csv'
...
fileName = 'Linear/E_GlassOldLinearTGA10KW_0_cone_Mass.csv'
df = pd.read_csv(fileName, skiprows=0)
dfMassTGA = df[['# Time [s]',' Mass [Kg]']]  # select time and MLR for 1D plot
dfMassTGA = np.transpose(np.array(dfMassTGA))

fileName = 'Linear/E_GlassOldLinearHybrid10KW_0_cone_Mass.csv'
df = pd.read_csv(fileName, skiprows=0)
...
# Save figures
plt.tight_layout()
plt.savefig('E_Glass_Linear_10kW_Mass_Comparison_TGA.png', dpi=300)
plt.show()
plt.close()
