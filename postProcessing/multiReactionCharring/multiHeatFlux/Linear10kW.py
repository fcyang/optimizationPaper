...
fileName = 'multiReacCharLinearNominal10KW_0_cone_Mass.csv'
...
fileName = 'multiReacCharLinearWide10KW_0_cone_Mass.csv'
df = pd.read_csv(fileName, skiprows=0)
dfMassWide = df[['# Time [s]',' Mass [Kg]']]  # select time and MLR for 1D plot
dfMassWide = np.transpose(np.array(dfMassWide))

fileName = 'multiReacCharLinearHybrid10KW_0_cone_Mass.csv'
df = pd.read_csv(fileName, skiprows=0)
...
# Save figures
plt.tight_layout()
plt.savefig('multi_Reaction_Linear_10KW_Mass_Comparison.png', dpi=300)
plt.show()
